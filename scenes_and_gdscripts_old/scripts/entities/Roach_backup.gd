...
func _set_target_outside_screen():
	...
	if nav_region:
		var nav_poly = nav_region.navigation_polygon
		if nav_poly:
			for i in range(nav_poly.get_outline_count()): # Iterate through all outlines if multiple exist
				var outline = nav_poly.get_outline(i)
	...
	if exit_points.size() > 0:
		...
		# This might lead to roaches getting stuck if navmesh doesn't extend properly.
