  ...
  try {
    ...
    const response = await fetch(`${Deno.env.get("SUPABASE_URL")}/functions/v1/generate-comic-panel`, {
      ...
      body: JSON.stringify({
        ...
        corruption_level: (oracleState?.oracle_stability_status || 'pristine').toLowerCase() // Normalize the corruption level
      })
    });
    if (!response.ok) {
...
// Legacy corruption-level normalization removed – all callers now must supply canonical levels.
