...
// const GIRTH_EXCHANGE_RATE = 0.000075; // 1 Girth = 0.000075 $GIRTH tokens (for future use)
...
serve(async (req) => {
  ...
  try {
    ...
    const { session_id, player_address, action, player_state } = await req.json() as StateRequest;
    ...
    if (action === 'save') {
      if (!player_state) {
        return new Response(
          JSON.stringify({ error: 'Player state required for save operation' }),
          { 
            status: 400, 
      ...
      if (saveError) {
        ...
        return new Response(
          JSON.stringify({ error: 'Failed to save player state', details: saveError.message }),
          { 
            status: 500, 
            headers: { ...corsHeaders, 'Content-Type': 'application/json' }
          }
      ...
      if (loadError && loadError.code !== 'PGRST116') { // PGRST116 = no rows found
        console.error(`❌ [STATE-MANAGER] Load error:`, loadError);
        return new Response(
          JSON.stringify({ error: 'Failed to load player state', details: loadError.message }),
          { 
            status: 500, 
            headers: { ...corsHeaders, 'Content-Type': 'application/json' }
          }
      ...
      return new Response(
        JSON.stringify({ error: 'Invalid action. Must be "load" or "save"' }),
        { 
          status: 400, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' }
        }
