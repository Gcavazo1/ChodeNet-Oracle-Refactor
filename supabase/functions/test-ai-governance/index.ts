...
async function createTestDecision(decisionData) {
...
async function triggerProphetAgent(decisionId) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Prophet Agent returned ${response.status}: ${errorText}`);
    }
...
async function testEmergencyBrake() {
  ...
  if (prophetResult.poll_result?.status !== 'blocked_emergency_brake') {
    throw new Error('Emergency brake did not block poll creation as expected');
  }
