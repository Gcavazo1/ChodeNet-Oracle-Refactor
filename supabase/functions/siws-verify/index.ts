...
serve(async (req) => {
  ...
  try {
    ...
    if (!message || !signature || !wallet_address) {
      ...
      return new Response(
        JSON.stringify({ error: 'Missing required fields: message, signature, wallet_address' }),
        { 
          status: 400, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' }
        }
    ...
    if (!nonceMatch) {
      ...
      return new Response(
        JSON.stringify({ error: 'Invalid SIWS message format - missing nonce' }),
        { 
          status: 400, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' }
        }
    ...
    if (existingProfile) {
      ...
      const { error: shardsError } = await supabaseAdmin
        .from('oracle_shards')
        .insert({
          user_profile_id: userProfile.id,
          balance: 0,
    ...
    const { data: girthBalance } = await supabaseAdmin
      .from('girth_balances')
      .select('*')
    ...
    const { data: oracleShards } = await supabaseAdmin
      .from('oracle_shards')
      .select('*')
      .eq('user_profile_id', userProfile.id)
      .single()
