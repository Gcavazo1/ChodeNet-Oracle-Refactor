...
serve(async (req) => {
  ...
  try {
    ...
    const { anonymous_session_id, wallet_address } = await req.json() as MigrationRequest;

    if (!anonymous_session_id || !wallet_address) {
      return new Response(JSON.stringify({ error: 'Missing required fields: anonymous_session_id and wallet_address' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      });
    }
    ...
    const { data: anonymousState, error: stateError } = await supabaseAdmin
      .from('player_states')
      .select('*')
      .eq('session_id', anonymous_session_id)
      .eq('is_anonymous', true)
      .single();
    
    if (stateError) {
      // It's possible the state was already claimed or never existed.
      if(stateError.code === 'PGRST116') { // Not found, which is not a server error
    ...
    if (anonymousState.current_girth > 0) {
      ...
      const { data: balanceRow, error: fetchBalanceError } = await supabaseAdmin
        .from("girth_balances")
