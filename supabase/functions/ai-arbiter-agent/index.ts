...
interface ArbiterAnalysis {
  ...
  voteMetrics: {
    ...
    demographicAnalysis: {
      vote_distribution: Array<{ option_id: string; votes: number; percentage: number }>;
      voting_patterns: string[];
...
serve(async (req) => {
  ...
  try {
    ...
    const { data: pollsNeedingAnalysis, error: pollsError } = await supabase
      .rpc('get_polls_needing_analysis');
    ...
    for (const pollSummary of pollsNeedingAnalysis || []) {
      ...
      try {
        ...
        const { data: pollData, error: pollDataError } = await supabase
          .rpc('get_poll_results_for_analysis', { target_poll_id: pollSummary.poll_id });
        ...
        const analysis = await generateArbiterAnalysis(pollData as PollAnalysisData);
        
        // Store analysis results
        const analysisResult = await storeAnalysisResults(supabase, pollData as PollAnalysisData, analysis);
        
        // Generate Oracle commentary
        const commentary = await generateOracleCommentary(supabase, pollData as PollAnalysisData, analysis);
        
        // Mark poll as analyzed
        await supabase.rpc('mark_poll_analyzed', { target_poll_id: pollSummary.poll_id });
...
async function generateArbiterAnalysis(pollData: PollAnalysisData): Promise<ArbiterAnalysis> {
  // Find winning option
  const winningOption = pollData.options.reduce((prev, current) => 
  ...
  const controversyScore = sortedOptions.length > 1 && totalVotes > 0 
    ? 1 - Math.abs(sortedOptions[0].percentage - sortedOptions[1].percentage) / 100
    : 0;
  ...
  return {
    winningOption: {
      ...
      margin_of_victory: sortedOptions.length > 1 
        ? winningOption.percentage - sortedOptions[1].percentage 
    ...
    rewardDistribution: {
      ...
      girth_pool_winners: pollData.poll.girth_reward_pool 
        ? voterWallets.filter(wallet => 
            pollData.voters.voting_timeline.find(v => v.wallet === wallet)?.option_id === winningOption.option_id
          )
        : undefined
    },
...
async function generateImplementationRecommendation(
  pollData: PollAnalysisData, 
  winningOption: any
): Promise<ArbiterAnalysis['implementationRecommendation']> {
  const prompt = `
...
Respond with valid JSON only.`
  ...
  try {
    ...
    return {
      ...
      requiredResources: analysis.requiredResources || ['Development team', 'Testing resources'],
      risks: analysis.risks || ['Standard implementation risks'],
      successMetrics: analysis.successMetrics || ['Community satisfaction', 'System stability'],
    ...
    return {
      ...
      requiredResources: ['Development team', 'Community coordination'],
      risks: ['Implementation complexity', 'Community reaction'],
      successMetrics: ['Successful deployment', 'Positive community feedback'],
      confidence_score: 0.75
    };
  }
}

async function generateOracleCommentaryText(
  pollData: PollAnalysisData,
  winningOption: any,
...
Respond with 2-3 paragraphs of Oracle commentary (no JSON, just the mystical text).`
  ...
  try {
    ...
    return fallbackCommentaries[personality as keyof typeof fallbackCommentaries] || fallbackCommentaries.chaotic_sage;
  }
}

function analyzeVotingPatterns(votingTimeline: any[]): string[] {
  const patterns = [];
  
  if (votingTimeline.length === 0) {
    return ['No voting activity detected'];
  }
...
async function generateOracleCommentary(
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      priority_level: analysis.implementationRecommendation.priority === 'immediate' ? 'critical' :
                    analysis.implementationRecommendation.priority === 'scheduled' ? 'high' : 'medium',
