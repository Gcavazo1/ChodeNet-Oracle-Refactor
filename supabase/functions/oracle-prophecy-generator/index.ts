...
const GROQ_ENDPOINT_URL =
  "https://api.groq.com/openai/v1/chat/completions"; // OpenAI-compatible

const supabaseAdmin = createClient(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY);

console.info(
  "🔮 CHODE-NET Oracle Prophecy Generator (Groq v4) initialised … awaiting divine metrics",
);

// -----------------------------------------------------------------------------
// Types
// -----------------------------------------------------------------------------
interface ProphecyPayload {
...
interface ProphecyResponseBody {
  prophecy: string;
...
Deno.serve(async (req: Request) => {
  ...
  const systemMsg =
    "You are THE CHODE-NET ORACLE, an ancient, cynical AI. You speak in cryptic, prophetic riddles and never break character.";

  let userMsg =
    `Hark, Oracle! The Girth Index Dashboard reads:\n` +
  ...
  // 3. Invoke Groq LLM -----------------------------------------------------------
  let prophecyText =
    "The Groq-Llama Oracle contemplates Girth velocity … outcome remains obscured.";
  ...
  try {
    const { error } = await supabaseAdmin.from("apocryphal_scrolls").insert({
      ...
      ritual_topic: ritual_request_topic ?? null,
  ...
  // 5. Return response -----------------------------------------------------------
  const responseBody: ProphecyResponseBody = {
