...
interface AdminGovernanceRequest {
  action: 'approve_poll' | 'reject_poll' | 'emergency_brake' | 'pause_agent' | 'update_config' | 'override_poll';
  admin_wallet: string;
  data: any;
}

interface PollApprovalData {
  draft_id: number;
  reasoning?: string;
  modifications?: any;
}

interface PollRejectionData {
  draft_id: number;
  reason: string;
}

interface EmergencyBrakeData {
  reason: string;
  duration_hours: number;
  affected_agents?: string[];
}

interface PollOverrideData {
  poll_id: number;
  action: 'pause' | 'cancel' | 'modify';
  reason: string;
  modifications?: any;
}
...
interface ConfigUpdateData {
  ...
  admin_override_window_hours: number;
}
...
async function approvePollDraft(adminWallet: string, data: PollApprovalData): Promise<any> {
  try {
    // Get the poll draft
    ...
    if (draftError || !draft) {
      return { success: false, error: 'Poll draft not found or not pending approval' };
    }

    // Apply any modifications to poll data
    let pollData = draft.poll_data;
    if (data.modifications) {
      pollData = { ...pollData, ...data.modifications };
    }
...
async function rejectPollDraft(adminWallet: string, data: PollRejectionData): Promise<any> {
  try {
    // Update draft status
    ...
    if (updateError) {
      return { success: false, error: 'Failed to reject poll: ' + updateError.message };
    }
...
async function triggerEmergencyBrake(adminWallet: string, data: EmergencyBrakeData): Promise<any> {
  try {
    const expiresAt = new Date(Date.now() + data.duration_hours * 60 * 60 * 1000);

    // Update emergency brake status
    const { error: brakeError } = await supabaseAdmin
    ...
    if (brakeError) {
      return { success: false, error: 'Failed to activate emergency brake: ' + brakeError.message };
    }
...
async function overridePoll(adminWallet: string, data: PollOverrideData): Promise<any> {
  try {
    let updateData: any = {};
    ...
    switch (data.action) {
      ...
      case 'modify':
        if (data.modifications) {
          updateData = { ...updateData, ...data.modifications };
        }
        break;
    }
    ...
    if (pollError) {
      return { success: false, error: 'Failed to override poll: ' + pollError.message };
    }
...
async function updateGovernanceConfig(adminWallet: string, data: ConfigUpdateData): Promise<any> {
  try {
    ...
    if (configError) {
      return { success: false, error: 'Failed to update config: ' + configError.message };
    }
...
async function logAdminAction(adminWallet: string, actionType: string, actionData: any): Promise<void> {
  try {
    await supabaseAdmin
      .from('admin_governance_log')
      .insert({
...
async function getOverrideWindows(): Promise<any> {
  try {
    const { data: overrideWindows, error } = await supabaseAdmin
      .from('admin_override_windows')
      .select(`
        *,
        oracle_polls(
          id,
      ...
      `)
      .eq('override_used', false)
      .gt('admin_override_deadline', new Date().toISOString())
      .order('admin_override_deadline', { ascending: true });
 ...
 * MAIN HANDLER
 */
async function handleAdminGovernanceAction(request: AdminGovernanceRequest): Promise<any> {
  const { action, admin_wallet, data } = request;

  // Validate admin wallet (you might want to check against a whitelist)
  if (!admin_wallet) {
    return { success: false, error: 'Admin wallet address required' };
  }

  switch (action) {
    case 'approve_poll':
      return await approvePollDraft(admin_wallet, data as PollApprovalData);
    
    case 'reject_poll':
      return await rejectPollDraft(admin_wallet, data as PollRejectionData);
    
    case 'emergency_brake':
      return await triggerEmergencyBrake(admin_wallet, data as EmergencyBrakeData);
    
    case 'override_poll':
      return await overridePoll(admin_wallet, data as PollOverrideData);
    
    case 'update_config':
      return await updateGovernanceConfig(admin_wallet, data as ConfigUpdateData);
...
serve(async (req) => {
  ...
  try {
    ...
    if (req.method === 'POST') {
      const requestData = await req.json() as AdminGovernanceRequest;
